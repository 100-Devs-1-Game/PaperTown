...
func save_mesh(
	...
	if reloaded_mesh:
		...
		push_error("failed to reload mesh? ", new_name)

	print("resaved mesh '%s'" % new_name)
...
func save_user_materials(
	...
	for i in range(surface_count):
		...
		if not material:
			push_error("missing material for mesh '%s'" % root.get_path_to(mesh_instance))
			return
		...
		if mat_name == "":
			print("material for mesh '%s' has an empty name" % root.get_path_to(mesh_instance))

		if _is_auto_generated_name(mat_name):
			print(
				(
					"found autogenerated name '%s' of material belonging to mesh '%s'"
					% [mat_name, root.get_path_to(mesh_instance)]
				)
			)
		...
		if overwrite_materials or not ResourceLoader.exists(mat_save_path):
			print("saving material to '%s'" % mat_save_path)
...
func _post_import(scene: Node):
	...
	# save again, to make sure our other changes to mats etc. are saved along with the mesh
	if save_meshes:
		for mesh_instance in mesh_instances:
