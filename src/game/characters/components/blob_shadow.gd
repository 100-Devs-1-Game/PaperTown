...
func _physics_process(_delta: float):
	var start := (get_parent() as Node3D).global_transform.origin + Vector3.UP * 0.1
	var end := (get_parent() as Node3D).global_transform.origin + Vector3.DOWN * raycast_distance
